...
Validates:
- YAML front matter (ai_task_list block)
- required headings
...
- required TDD headings + STOP gate
...
RE_ARRAY_START = re.compile(r"^TASK_(\d+)_(\d+)_PATHS=\s*\(\s*$")
...
def lint(path: Path) -> Tuple[Dict[str, str], List[LintError]]:
    ...
    b_start = next((i for i, ln in enumerate(content) if ln == baseline_heading), None)
    if b_start is not None:
        b_end = next((i for i in range(b_start + 1, len(content)) if content[i].startswith("## ")), len(content))
    ...
    d_start = next((i for i, ln in enumerate(content) if ln == drift_heading), None)
    if d_start is not None:
        d_end = next((i for i in range(d_start + 1, len(content)) if content[i].startswith("## ")), len(content))
        sec = content[d_start:d_end]
        header = next((ln for ln in sec if ln.strip().startswith("|")), None)
        if header is None:
    ...
    g_start = next((i for i, ln in enumerate(content) if ln == g_heading), None)
    if g_start is not None:
        g_end = next((i for i in range(g_start + 1, len(content)) if content[i].startswith("## ")), len(content))
...
def main(argv: List[str]) -> int:
    parser = argparse.ArgumentParser(prog="ai_task_list_linter_v1.py")
