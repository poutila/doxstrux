#!/usr/bin/env python3
"""
ai_task_list_linter_v1_5.py

Deterministic linter for AI Task Lists (Spec v1.3).

Version 1.5 changes (closes all remaining gaps):
- FIX Gap A: STOP evidence labels (# Test run output: / # Symbol/precondition check output:)
  are now REQUIRED in instantiated mode, not just when --require-captured-evidence is used
- FIX Gap B: Captured header metadata lines (# cmd:, # exit:, # ts_utc:, # cwd:) no longer
  count as "evidence content" - real output lines are required
- FIX Gap C: UV_FORBIDDEN patterns only checked on $ command lines, not pasted output
- FIX Gap D: Phase Unlock Artifact must contain actual artifact command, not just prose mention

Version 1.4 features retained:
- R-ATL-024: Captured evidence headers (opt-in via --require-captured-evidence)
- R-ATL-071: Runner prefix on $ command lines only
- R-ATL-033: Naming rule exactly once
...
def lint(path: Path, require_captured_evidence: bool = False) -> Tuple[Dict[str, str], List[LintError]]:
    ...
    # R-ATL-050: Phase Unlock Artifact content enforcement (Gap D fix: stronger enforcement)
    ...
    if p_bounds:
        ...
        # Must have artifact file pattern in a code block or command context
        # Pattern: .phase-N.complete.json or .phase-0.complete.json etc
        ...
        if meta["mode"] == "template":
            ...
            # Instantiated mode: must have concrete artifact with specific phase number
            specific_artifact = re.search(r"\.phase-\d+\.complete", sec_text, re.IGNORECASE)
            if not specific_artifact:
                errors.append(LintError(content_start + 1 + p_start, "R-ATL-050",
                    "Phase Unlock Artifact must include specific artifact file (e.g., .phase-0.complete.json, .phase-1.complete.json)."))
...
def main(argv: List[str]) -> int:
    parser = argparse.ArgumentParser(prog="ai_task_list_linter_v1_5.py")
