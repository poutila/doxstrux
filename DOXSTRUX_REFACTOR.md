...
## Phase A — Day 1: Core Infrastructure (6-8 hours)
**Goal**: Complete index building + section lookup + helper methods

### Step 1: Implement `_build_indices()` in `token_warehouse.py`
...
```bash
pytest -q tests/test_token_warehouse_indices.py::test_pairs_bidirectional
```

Verify: Indices populate pairs/sections/parents correctly in a single pass.

### Step 2: Add `section_of(line)` with binary search (O(log N))

```bash
pytest -q tests/test_section_lookup_performance.py
```

Verify: Binary search returns the correct section within 1 ms for 10k-line documents.

### Step 3: Add helper methods + unit tests
- `find_close(idx)`, `find_parent(idx)`, `find_children(idx)`
- `tokens_between(a, b, type=None)` (O(logN + K) via bisect over sorted type-index)
- `text_between(a, b, join_spaces=True)`; for headings use `join_spaces=False`
- `get_line_range(start, end)`, `get_token_text(idx)`

```bash
pytest -q tests/test_token_warehouse_adversarial.py
```

Verify: Helper methods return expected ranges and honor normalization.

Gate: All index tests green before Day 2.

---

## Phase B — Day 2: Dispatch + Collector Migration (6-8 hours)
**Goal**: Single-pass O(N+M) dispatch + prove pattern with 2-3 collectors

### Step 1: Implement routing table in `register_collector()`
- Build `self._routing: dict[str, list[Collector]]` (order-preserving)
- Support type-based and tag-based routing
- **No `set()` dedup**—append only if not present to preserve registration order

### Step 2: Rewrite `dispatch_all()` for single-pass
- One iteration over tokens; O(1) routing lookup per token
...
```bash
pytest -q tests/test_dispatch.py::test_dispatch_single_pass
```

Verify: Each token visited once; routing preserves registration order; no reentrancy; timeout guard active.

### Step 3: Migrate 2–3 representative collectors to index-first
- **LinksCollector**: Use `find_close()`, `text_between()`
- **ImagesCollector**: Similar to links
- **HeadingsCollector**: Use `sections` index
- Ban `for tok in wh.tokens` iteration (monkeypatch in tests)

```bash
pytest -q tests/test_collectors_links.py::test_links_index_first \
         tests/test_collectors_headings.py::test_headings_sections
```

Verify: Collectors never iterate `wh.tokens`; outputs match fixtures; dispatch complexity proves O(N+M).

---

## Phase C — Day 3: API Shim + Baseline Parity (8-10 hours)
**Goal**: Drop-in compatibility + baseline test fixes

### Step 1: Create `markdown/parser.py` with `MarkdownParserCore`
- Match src API signature exactly
- Register all 12 collectors
- `parse()` returns src-compatible dict
- Add all `extract_*()` methods
- Wire all collectors into parser

### Step 2: Run baseline test suite and perf smoke
...
Verify: ≥80% baseline tests passing; no critical perf regressions (>10%).

### Step 3: Triage + decision
- Fix first wave of failures (focus on output structure differences)
- Document blockers for remaining failures
- Create issue list for Day 4+ work
- Decision Point: If <80% parity, fall back to detailed plan
