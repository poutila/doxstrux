...
def enforce_pin_sec_010(path: Path, text: str) -> List[LintError]:
    """Enforce PIN-SEC-010: External Dependencies MUST contain `### Required Files/Modules`"""
    return check_heading_exists(
        path, text,
        rule_id="PIN-SEC-010",
        heading="### Required Files/Modules",
        exact_match=False
    )
