...
- Spec version bump to 1.6

Version 1.7 features retained:
- R-ATL-001: schema_version == "1.6", search_tool required
...
def _parse_front_matter(lines: List[str]) -> Tuple[Dict[str, str], Optional[LintError], int]:
    ...
    if meta["schema_version"] != "1.6":
        return meta, LintError(1, "R-ATL-001", f"schema_version must be '1.6' (found '{meta['schema_version']}')."), end_idx + 1
...
def main(argv: List[str]) -> int:
    parser = argparse.ArgumentParser(prog="ai_task_list_linter_v1_8.py")
