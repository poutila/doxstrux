...
- `AI_TASK_LIST_SPEC_v1.md` — the contract (Spec v1.6).
- `AI_TASK_LIST_TEMPLATE_v6.md` — starting point for new task lists.
- `ai_task_list_linter_v1_8.py` — the linter implementing the spec (v1.8).
- `README_ai_task_list_linter_v1_8.md` — release notes and highlights.
- Orchestrator prompts:
  - `PROMPT_AI_TASK_LIST_ORCHESTRATOR_v1.md` — runtime prompt for converting prose → AI task list (template mode output).

## 1) Core Concepts
- SSOT: spec + linter are authoritative. If this manual/template ever disagree with them, the spec/linter win.
- Modes: `template` allows placeholders; `instantiated` forbids them.
...
2. Fill YAML front matter: `schema_version: "1.6"`, `mode: "template"` while designing; set `runner`, `runner_prefix`, `search_tool`.
