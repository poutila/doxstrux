...
**Target set:** All markdown documents (`**/*.md`) under:

- `/home/lasse/Dropbox/python/omat/doxstrux/files (9)`

**Input contract (no guessing):**
- You MUST be given the full text of every target document in this run (e.g., via ZIP upload or pasted text).
- If any target document text is missing, report a **BLOCKER** and STOP the entire run (do not continue with partial coverage).

**Target artifacts you may use ONLY:**
- The Target documents’ text (as provided in this run)
- These attached framework artifacts:
  - `COMMON.md`
  ...
  - `VALIDATION_SUITE.md`
  - `INDEX.md`
  - `README_ai_task_list_linter_v1_9.md`

**No other assumptions allowed:**
- Do not assume filesystem structure beyond what is documented in the provided texts
- Do not assume tooling beyond what is explicitly specified in the provided texts
- Do not assume internal conventions not present in the provided texts
- Do not assume any context not present in the provided texts
...
1. **Version comparisons / legacy references**
   - Mentions of older versions or comparisons (e.g., “unlike v1.6”, “improved from v1.8”)
