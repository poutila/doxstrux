#!/usr/bin/env python3
"""
ai_task_list_linter_v1_3.py

Deterministic linter for AI Task Lists (Spec v1.1).

Version 1.3 changes (eliminates drift):
- D4 fix: grep detection now only inside fenced code blocks (no false positives on prose)
- R-ATL-033: Enforces naming rule stated once (Task N.M → TASK_N_M_PATHS)
- R-ATL-023: Non-empty evidence required in instantiated mode
  - Baseline Snapshot evidence must have output after $ commands
  - Per-task STOP evidence must have non-empty content
  - Global Clean Table evidence must have non-empty content
- R-ATL-071 strengthened: Runner prefix checked on all commands in required blocks
...
Validates:
- YAML front matter (ai_task_list block, search_tool field)
...
- naming rule stated once (R-ATL-033)
- required TDD headings + STOP gate (Phase 1+ only)
- No Weak Tests prompts
- Non-empty evidence in instantiated mode (R-ATL-023)
...
- search_tool enforcement in code blocks only (D4)
- runner enforcement on all command lines (R-ATL-071/072)
"""
...
def lint(path: Path) -> Tuple[Dict[str, str], List[LintError]]:
    ...
    # Look for the naming convention statement
    naming_rule_patterns = [
        r"Naming rule.*TASK_N_M_PATHS",
        r"Task.*N\.M.*TASK_N_M_PATHS",
        r"TASK_\[\[PH:.*\]\]_PATHS",  # Template version
    ]
    ...
    if meta["mode"] == "instantiated":
        ...
        if prefix:
            # R-ATL-071 strengthened: Check all command lines in code blocks
            # Commands that need runner_prefix: pytest, python, mypy, ruff, black, isort
            runner_commands = ["pytest", "python", "mypy", "ruff", "black", "isort"]
            # Commands that don't need prefix: git, rg, grep, cd, ls, cat, echo, test, mkdir, cp, mv, rm
            exempt_commands = ["git", "rg", "grep", "cd", "ls", "cat", "echo", "test", "mkdir", "cp", "mv", "rm", "touch", "for", "if", "done", "fi"]
            ...
            for idx, ln in enumerate(content):
                ...
                if stripped.startswith("```") or stripped.startswith("#") or not stripped:
                    continue
                # Skip lines that start with $ (these are command prompts, check the actual command)
                ...
                for cmd in runner_commands:
                    if cmd in stripped:
                        ...
                        if prefix not in ln:
                            # Make sure this isn't just mentioning the command in a comment or output
                            # Check if the command appears at start or after common shell operators
                            cmd_pattern = rf'(^|\s|&&|\|\||;|\$)\s*{cmd}\b'
                            if re.search(cmd_pattern, stripped):
...
def main(argv: List[str]) -> int:
    parser = argparse.ArgumentParser(prog="ai_task_list_linter_v1_3.py")
