...
grep -E '\[\[PH:|YYYY-MM-DD|TBD' .phase-0.complete.json && exit 1 || true
```
...
grep -E '\[\[PH:|YYYY-MM-DD|TBD' .phase-N.complete.json && exit 1 || true
```
...
# Example for Python:
# grep -RInE 'TODO|FIXME|XXX|\[\[PH:' --include='*.py' --include='*.md' . && exit 1 || true
