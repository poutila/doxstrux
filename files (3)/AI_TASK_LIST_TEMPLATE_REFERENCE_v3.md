...
**The template**: `AI_TASK_LIST_TEMPLATE_v3.md` (~280 lines, execution-focused)

> **Non-normative**: This guide is for human understanding only. The linter validates the template (or instantiated task list), not this guide. Examples of document names are illustrative — actual names belong in your instantiated task list.
...
## Mode Validation (for linter)

The YAML front matter `mode` field determines linter behavior:

| Mode | Placeholders | Validation |
|------|--------------|------------|
| `template` | Allowed | Structure only (sections, headings, arrays) |
| `instantiated` | Forbidden | Structure + no `[[PH:` + required evidence |

**Linter pseudo-logic**:
```python
if metadata.mode == "template":
    validate_structure_only()
elif metadata.mode == "instantiated":
    validate_structure()
    assert_no_placeholders()  # grep -RInE '\[\[PH:' must be zero
    assert_evidence_present()  # PASTE sections filled
    validate_runner_consistency(metadata.runner)
```
...
## Runner Enforcement via Metadata (for linter)

The template uses YAML front matter to declare runner:
...
**Linter rules by runner**:

| runner | Forbidden patterns | Required patterns |
|--------|-------------------|-------------------|
| `uv` | `.venv/bin/python`, `python -m`, `pip install` | `uv run`, `uv sync`, `uv add` |
| `poetry` | `.venv/bin/python`, `pip install` | `poetry run`, `poetry install` |
| `npm` | Direct `node` calls | `npm run`, `npm install` |

This keeps the template general while enabling strict enforcement in instantiated task lists.
