...
class MarkdownTestFramework:
    ...
    def extract_feature_counts(self, result: dict, parser=None) -> dict[str, int]:
        ...
        # HTML blocks and inline HTML - now in separate structures
        ...
        # Footnote quality metrics
        footnotes = structure.get("footnotes", {})
        if isinstance(footnotes, dict):
            # Count duplicate footnote labels (if parser provides this info)
        ...
        table_metrics = self._validate_table_quality(structure)
        counts.update(table_metrics)

        return counts

    def _validate_table_quality(self, structure) -> dict[str, int]:
        """Validate table structural integrity and return quality metrics."""
        tables = structure.get("tables", [])
        metrics = {"table_align_mismatches": 0, "table_ragged_rows": 0}
        ...
        for table in tables:
            ...
            if header_count > 0 and align_count > 0 and header_count != align_count:
                metrics["table_align_mismatches"] += 1
            ...
            if rows:
                ...
                if expected_width > 0:
                    ...
                    if mismatched_rows:
                        metrics["table_ragged_rows"] += len(mismatched_rows)
    ...
    def _count_hrules_from_tokens(self, parser) -> int:
        ...
        # Use the parser's new method if available (includes YAML frontmatter delimiters)
        if hasattr(parser, "get_total_hrule_count"):
            return parser.get_total_hrule_count()

        # Fallback to old method for backward compatibility
        if not hasattr(parser, "tokens"):
            return 0
...
def main():
    ...
    print("🔬 Enhanced MarkdownParserCore Test Framework")
    print("=" * 70)
    ...
    print("\n" + "=" * 70)
    print("🔄 Starting comprehensive test suite with enhanced metrics...")
