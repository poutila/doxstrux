# AI Task List Spec v1.2

**Spec ID**: `AI_TASK_LIST_SPEC_V1_2`  
**Schema version**: `1.2`  
...
6. Verifiable evidence provenance via captured headers (v1.2, opt-in)
...
### R-ATL-023: Non-empty evidence required (instantiated mode) — NEW in v1.1

If `mode: instantiated`, every required evidence slot MUST contain at least one non-empty output line.

This applies to:

1. **Baseline Snapshot evidence**: Each `$ command` line must have at least one non-empty output line following it.

2. **Per-task STOP evidence** (non-Phase-0): The evidence block must contain non-empty content under both:
   - `# Test run output:`
   - `# Symbol/precondition check output:`

3. **Global Clean Table evidence**: The evidence block must contain at least one non-empty line.

**Rationale**: Eliminates the "blank evidence" loophole where a structurally valid document passes lint but contains no actual proof of execution.

### R-ATL-024: Captured evidence headers (opt-in) — NEW in v1.2
...
### R-ATL-050: Phase unlock artifact section required — ENFORCED in v1.2
...
**Linter enforcement**:
- Template mode: Section must describe artifact generation or placeholder scanning
- Instantiated mode: Section MUST reference phase artifact file pattern (e.g., `.phase-0.complete.json`, `.phase-1.complete.json`)

**Rationale**: The Phase Unlock Artifact is the gate between phases. Without explicit artifact references, the phase gating contract becomes unenforceable.
...
A document is **Spec v1.2 compliant** if:

1. ✅ Has required YAML metadata (schema_version: "1.2") and required headings (9 total)
...
7. ✅ Non-empty evidence in instantiated mode (R-ATL-023)
8. ✅ **NEW**: Phase Unlock Artifact references artifact file in instantiated mode (R-ATL-050)
...
11. ✅ Runner prefix enforced on $ command lines only (R-ATL-071 — fixed false positives)
12. ✅ Runner-specific rules enforced (R-ATL-072 for uv)
