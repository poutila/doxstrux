...
def test_allow_raw_html_flag_documented():
    ...
    doc_path = Path("docs/ALLOW_RAW_HTML_POLICY.md")
...
```markdown
# FILE: docs/ALLOW_RAW_HTML_POLICY.md
...
HTMLCollector is **default-off** (fail-closed). Enabling raw HTML requires:
...
- **Ticket ID**: Required (tracks requirement)
- **Approver**: Tech Lead
- **Expires**: 30 days (auto-expire)
- **Risk Assessment**: Must document XSS risk acceptance
...
All changes to HTMLCollector must pass:
```bash
pytest skeleton/tests/test_html_render_litmus.py
```
```

**Success Criteria**:
- [ ] All 4 litmus tests pass
- [ ] ALLOW_RAW_HTML_POLICY.md created
...
```markdown
# FILE: docs/PLATFORM_SUPPORT_POLICY.md
...
### Limited Support (Graceful Degradation)
- ⚠️ Windows (timeout warning only, no enforcement)

### Windows Deployment Options

**Option 1: Graceful Degradation (Default)**
- Timeout warnings logged but not enforced
- Suitable for trusted collector code
- Set: `STRICT_TIMEOUT_ENFORCEMENT=false` (default)

**Option 2: Subprocess Isolation (Strict Mode)**
- Requires implementation (pending YAGNI waiver)
- Set: `STRICT_TIMEOUT_ENFORCEMENT=true`
- Raises `NotImplementedError` until implemented

**Option 3: Linux-Only Deployment (Recommended for Untrusted Inputs)**
- Deploy on Linux for production
- Use Windows only for development/testing with trusted collectors
...
**Current Status**: NOT IMPLEMENTED (awaiting waiver)

## Testing

```bash
# Unix (enforced)
pytest skeleton/tests/test_collector_timeout.py -v
# Expected: All tests pass

# Windows (graceful)
set STRICT_TIMEOUT_ENFORCEMENT=false
pytest skeleton/tests/test_collector_timeout.py -v
# Expected: Warnings logged, tests skip timeout assertions
```
```
...
**Success Criteria**:
- [ ] Platform support explicitly documented (PLATFORM_SUPPORT_POLICY.md)
...
- [ ] **P0-2: HTML/XSS Litmus**
  ...
  - [ ] `ALLOW_RAW_HTML_POLICY.md` exists and reviewed
  - [ ] HTMLCollector.allow_html defaults to False
...
- [ ] **P0-4: Cross-Platform Isolation**
  - [ ] `PLATFORM_SUPPORT_POLICY.md` exists
