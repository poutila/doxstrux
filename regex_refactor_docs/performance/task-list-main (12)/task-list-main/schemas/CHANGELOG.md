...
### Added
- `schemas/phase_complete.schema.json` to validate `.phase-N.complete.json` artifacts with numeric counters and ISO 8601 completion timestamps.
...
### Changed
- Updated `schemas/detailed_task_list.schema.json` to advertise `model_version` 2.0.0 and keep metadata in lockstep with the
...
- Ensure renderers write the new `render_meta` block and update
  `template_metadata.last_rendered_utc`.
