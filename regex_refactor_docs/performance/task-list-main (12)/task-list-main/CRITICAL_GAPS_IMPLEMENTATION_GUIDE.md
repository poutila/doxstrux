...
**Fix**: Create `src/tasklist/validate_task_ids.py`
...
**Add to renderer** (`src/tasklist/render_task_templates.py`):
...
tasklist-validate  # Should error
```
...
**Fix**: Create `src/tasklist/validate_commands.py`
...
**Fix**: Create `src/tasklist/sanitize_paths.py`
...
tasklist-validate && echo "PASS" || echo "FAIL"

# 4. Test command safety
tasklist-commands && echo "PASS" || echo "FAIL"

# 5. Test path sanitization
tasklist-sanitize && echo "PASS" || echo "FAIL"
...
- [x] `src/tasklist/validate_task_ids.py` exists and validates uniqueness + order
- [x] `src/tasklist/validate_commands.py` exists and rejects dangerous patterns
- [x] `src/tasklist/sanitize_paths.py` exists and prevents path traversal
