...
The GitHub Actions workflow re-renders the artifacts, validates the JSON schema, checks hash alignment, rejects unsafe shell commands, and ensures the working tree remains clean. It also validates any `.phase-*.complete.json` artifacts against `schemas/phase_complete.schema.json`.
