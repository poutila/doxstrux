...
def add_render_meta(
    ...
    *,
    ...
    if isinstance(existing_json, dict):
        ...
        prev_metadata = existing_json.get("metadata")
        if isinstance(prev_metadata, dict):
            prev_template_meta = prev_metadata.get("template_metadata")
    ...
    metadata_section = data.setdefault("metadata", {})
    if not isinstance(metadata_section, dict):
        metadata_section = {}
        data["metadata"] = metadata_section

    template_meta = metadata_section.setdefault("template_metadata", {})
    if not isinstance(template_meta, dict):
        template_meta = {}
        metadata_section["template_metadata"] = template_meta
...
def _gate_sort_key(gate: dict) -> tuple[int, tuple[int, ...] | str]:
    gate_id = str(gate.get("id", ""))
    if gate_id.startswith("G") and gate_id[1:].isdigit():
        return (0, (int(gate_id[1:]),))
    return (1, gate_id)
...
def _validate_phase_completion(root: Path) -> list[str]:
    """Validate `.phase-*.complete.json` artifacts against the packaged schema."""

    if not root.exists():
        return []

    completion_files = sorted(root.glob(".phase-*.complete.json"))
    if not completion_files:
        return []

    with importlib_resources.as_file(
        importlib_resources.files("tasklist.schemas").joinpath("phase_complete.schema.json")
    ) as schema_path:
        schema = json.loads(schema_path.read_text(encoding="utf-8"))

    validator = Draft202012Validator(schema)
    errors: list[str] = []
    for artifact in completion_files:
        try:
        ...
        for error in validator.iter_errors(payload):
            ...
            if pointer:
                pointer = f" ({pointer})"
            errors.append(f"❌ {artifact.name}{pointer}: {error.message}")
...
def render_markdown(data: dict) -> str:
    ...
    lines.append("render_meta:")
    lines.append(f"  source_file: \"{render_meta.get('source_file', 'Unknown')}\"")
    lines.append(f"  schema_version: \"{render_meta.get('schema_version', data.get('schema_version', '1.0.0'))}\"")
    lines.append(f"  sha256_of_yaml: \"{render_meta.get('sha256_of_yaml', 'Unknown')}\"")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    allow_tbd = args.allow_tbd or not args.strict
    unresolved = find_placeholders(data, allow_tbd=allow_tbd)
    if unresolved:
    ...
    validation_errors.extend(_validate_phase_completion(root))
