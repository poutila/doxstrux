...
These are the absolute minimum fixes to prevent catastrophic failures:
  ...
  verify-render:
    ...
    steps:
      ...
      - name: Re-render from YAML
        run: |
          cd regex_refactor_docs/performance/detailed_task_list_template
  ...
  - repo: local
    hooks:
      ...
      - id: scan-placeholders
        name: Scan for placeholder leakage
        entry: bash -c 'cd regex_refactor_docs/performance/detailed_task_list_template && ! grep -E "\{\{[^}]+\}\}" DETAILED_TASK_LIST_template.json'
        language: system
        files: '\.json$'
        pass_filenames: false
```
...
└── RISK_ANALYSIS_AND_MITIGATION.md # Reference
```
