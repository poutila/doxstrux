...
description: Detect Liskov Substitution Principle violations before committing. Use proactively to prevent NotImplementedError in subclasses and contract violations that break inheritance.
allowed-tools: Bash, Read, Task
---

# Solid Lsp Validator Skill

**This skill invokes the [solid-lsp-validator](../../agents/solid-lsp-validator.md) agent.**
...
1. **NotImplementedError** - Subclass doesn't implement parent method
2. **Contract violations** - Subclass breaks parent contract
3. **Signature changes** - Subclass changes method signature
...
```bash
> Use the solid-lsp-validator agent to verify my code
```

**Examples**:
```bash
> Use the solid-lsp-validator agent before committing

> Have the solid-lsp-validator agent check for issues

> Ask the solid-lsp-validator agent to analyze the codebase
```
...
```python
Task(
    subagent_type="solid-lsp-validator",
    description="Check code quality",
    prompt="Run solid-lsp-validator analysis and report all findings with exact locations."
)
```
...
# Run validation
echo "🔍 Running Solid Lsp Validator..."
uv run fact-file-query-tool solid-lsp
echo "✅ Solid Lsp Validator passed"
```
...
# Run validation
echo "🔍 Running Solid Lsp Validator..."
uv run fact-file-query-tool solid-lsp --format summary
echo "✅ Solid Lsp Validator passed"

uv run fact-file-query-tool solid-lsp --format json

uv run fact-file-query-tool solid-lsp --file path/to/file.py
```
...
**Agent file**: [solid-lsp-validator](../../agents/solid-lsp-validator.md)
...
**Invocation**:
```bash
> Use the solid-lsp-validator agent to verify my code before committing
```
...
Task(subagent_type="solid-lsp-validator", ...)
...
- **Agent**: [solid-lsp-validator](../../agents/solid-lsp-validator.md)
