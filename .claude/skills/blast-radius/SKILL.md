---
name: blast-radius
description: Analyze blast radius for incident response. Use proactively when investigating incidents, planning changes, or assessing impact. Shows who calls a function, what it calls, error contract, side effects - everything needed for fast triage.
allowed-tools: Bash, Read, Task
---

# Blast Radius Skill

**This skill invokes the [blast-radius](../../agents/blast-radius.md) agent.**
...
```bash
> Use the blast-radius agent to verify my code
```

**Examples**:
```bash
> Use the blast-radius agent before committing

> Have the blast-radius agent check for issues

> Ask the blast-radius agent to analyze the codebase
```
...
Task(
    ...
    prompt="Run blast-radius analysis and report all findings with exact locations."
)
```
...
# Run validation
echo "🔍 Running Blast Radius..."
uv run fact-file-query-tool blast-radius --format summary
echo "✅ Blast Radius passed"

uv run fact-file-query-tool blast-radius --format json

uv run fact-file-query-tool blast-radius --file path/to/file.py
```
...
**Agent file**: [blast-radius](../../agents/blast-radius.md)
...
**Invocation**:
```bash
> Use the blast-radius agent to verify my code before committing
```
...
Task(subagent_type="blast-radius", ...)
...
- **Agent**: [blast-radius](../../agents/blast-radius.md)
