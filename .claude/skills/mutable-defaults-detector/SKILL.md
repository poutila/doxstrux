...
description: Detect mutable default arguments before committing. Use proactively to catch critical Python anti-pattern of using lists, dicts, or sets as default parameters. Prevents subtle bugs where default values are shared across function calls.
allowed-tools: Bash, Read, Task
---

# Mutable Defaults Detector Skill

**This skill invokes the [mutable-defaults-detector](../../agents/mutable-defaults-detector.md) agent.**
...
- Lists: `def func(items=[])`
- Dicts: `def func(mapping={})`
- Sets: `def func(values=set())`
- Other mutables: `def func(obj=SomeClass())`
...
```bash
> Use the mutable-defaults-detector agent to verify my code
```

**Examples**:
```bash
> Use the mutable-defaults-detector agent before committing

> Have the mutable-defaults-detector agent check for issues

> Ask the mutable-defaults-detector agent to analyze the codebase
```
...
```python
Task(
    subagent_type="mutable-defaults-detector",
    description="Check code quality",
    prompt="Run mutable-defaults-detector analysis and report all findings with exact locations."
)
```
...
# Run validation
echo "🔍 Running Mutable Defaults Detector..."
uv run fact-file-query-tool mutable-defaults
echo "✅ Mutable Defaults Detector passed"
```
...
# Run validation
echo "🔍 Running Mutable Defaults Detector..."
uv run fact-file-query-tool mutable-defaults --format summary
echo "✅ Mutable Defaults Detector passed"

uv run fact-file-query-tool mutable-defaults --format json

uv run fact-file-query-tool mutable-defaults --file path/to/file.py
```
...
**Agent file**: [mutable-defaults-detector](../../agents/mutable-defaults-detector.md)
...
**Invocation**:
```bash
> Use the mutable-defaults-detector agent to verify my code before committing
```
...
- **Agent**: [mutable-defaults-detector](../../agents/mutable-defaults-detector.md)
