...
description: Detect Single Responsibility Principle violations before committing. Use proactively to prevent god classes, high coupling, and classes with too many methods. Maintains single responsibility.
allowed-tools: Bash, Read, Task
---

# Solid Srp Validator Skill

**This skill invokes the [solid-srp-validator](../../agents/solid-srp-validator.md) agent.**
...
1. **God classes** - Too many methods (>10)
2. **High coupling** - Too many dependencies (fan-out >10)
3. **Multiple responsibilities** - Violates SRP
...
```bash
> Use the solid-srp-validator agent to verify my code
```

**Examples**:
```bash
> Use the solid-srp-validator agent before committing

> Have the solid-srp-validator agent check for issues

> Ask the solid-srp-validator agent to analyze the codebase
```
...
Task(
    ...
    prompt="Run solid-srp-validator analysis and report all findings with exact locations."
)
```
...
# Run validation
echo "🔍 Running Solid Srp Validator..."
uv run fact-file-query-tool solid-srp
echo "✅ Solid Srp Validator passed"
```
...
# Run validation
echo "🔍 Running Solid Srp Validator..."
uv run fact-file-query-tool solid-srp --format summary
echo "✅ Solid Srp Validator passed"

uv run fact-file-query-tool solid-srp --format json

uv run fact-file-query-tool solid-srp --file path/to/file.py
```
...
**Agent file**: [solid-srp-validator](../../agents/solid-srp-validator.md)
...
**Invocation**:
```bash
> Use the solid-srp-validator agent to verify my code before committing
```
...
- **Agent**: [solid-srp-validator](../../agents/solid-srp-validator.md)
