...
description: Validate naming conventions before committing. Use proactively to enforce SCREAMING_SNAKE_CASE for constants, snake_case for functions/variables, PascalCase for classes. Ensures naming consistency across codebase.
allowed-tools: Bash, Read, Task
---

# Naming Validator Skill

**This skill invokes the [naming-validator](../../agents/naming-validator.md) agent.**
...
1. **Constants** - SCREAMING_SNAKE_CASE (MAX_SIZE, API_KEY)
2. **Functions/methods** - snake_case (process_data, get_user)
3. **Classes** - PascalCase (UserManager, DataProcessor)
4. **Private** - Leading underscore (_internal_func)
...
```bash
> Use the naming-validator agent to verify my code
```

**Examples**:
```bash
> Use the naming-validator agent before committing

> Have the naming-validator agent check for issues

> Ask the naming-validator agent to analyze the codebase
```
...
```python
Task(
    subagent_type="naming-validator",
    description="Check code quality",
    prompt="Run naming-validator analysis and report all findings with exact locations."
)
```
...
# Run validation
echo "🔍 Running Naming Validator..."
uv run fact-file-query-tool naming
echo "✅ Naming Validator passed"
```
...
# Run validation
echo "🔍 Running Naming Validator..."
uv run fact-file-query-tool naming --format summary
echo "✅ Naming Validator passed"

uv run fact-file-query-tool naming --format json

uv run fact-file-query-tool naming --file path/to/file.py
```
...
**Agent file**: [naming-validator](../../agents/naming-validator.md)
...
**Invocation**:
```bash
> Use the naming-validator agent to verify my code before committing
```
...
Task(subagent_type="naming-validator", ...)
...
- **Agent**: [naming-validator](../../agents/naming-validator.md)
