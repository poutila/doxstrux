...
description: Detect weak tests before committing. Use proactively when writing tests to ensure quality - catch import-only tests, tests with no assertions, exit-code-only tests that provide false confidence.
allowed-tools: Bash, Read, Task
---

# Weak Test Detector Skill

**This skill invokes the [weak-test-detector](../../agents/weak-test-detector.md) agent.**
...
1. **Import-only tests** - Just imports, no assertions
2. **No assertions** - Tests that don't verify anything
3. **Exit-code-only** - Rely on crash detection only
...
```bash
> Use the weak-test-detector agent to verify my code
```

**Examples**:
```bash
> Use the weak-test-detector agent before committing

> Have the weak-test-detector agent check for issues

> Ask the weak-test-detector agent to analyze the codebase
```
...
```python
Task(
    subagent_type="weak-test-detector",
    description="Check code quality",
    prompt="Run weak-test-detector analysis and report all findings with exact locations."
)
```
...
# Run validation
echo "🔍 Running Weak Test Detector..."
uv run fact-file-query-tool weak-tests
echo "✅ Weak Test Detector passed"
```
...
# Run validation
echo "🔍 Running Weak Test Detector..."
uv run fact-file-query-tool weak-tests --format summary
echo "✅ Weak Test Detector passed"

uv run fact-file-query-tool weak-tests --format json

uv run fact-file-query-tool weak-tests --file path/to/file.py
```
...
**Agent file**: [weak-test-detector](../../agents/weak-test-detector.md)
...
**Invocation**:
```bash
> Use the weak-test-detector agent to verify my code before committing
```
...
Task(subagent_type="weak-test-detector", ...)
...
- **Agent**: [weak-test-detector](../../agents/weak-test-detector.md)
