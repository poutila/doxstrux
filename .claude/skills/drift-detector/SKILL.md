---
name: drift-detector
description: Detect drift between FACTS snapshots for CI/CD gating. Use when checking for breaking changes, validating releases, or detecting documentation drift. Compares baseline vs current to identify signature changes and contract violations.
allowed-tools: Bash, Read, Task
---

# Drift Detector Skill

**This skill invokes the [drift-detector](../../agents/drift-detector.md) agent.**
...
```bash
> Use the drift-detector agent to verify my code
```

**Examples**:
```bash
> Use the drift-detector agent before committing

> Have the drift-detector agent check for issues

> Ask the drift-detector agent to analyze the codebase
```
...
Task(
    ...
    prompt="Run drift-detector analysis and report all findings with exact locations."
)
```
...
uv run fact-file-query-tool drift
echo "✅ Drift Detector passed"
```
...
# Run validation
echo "🔍 Running Drift Detector..."
uv run fact-file-query-tool drift --format summary
echo "✅ Drift Detector passed"

uv run fact-file-query-tool drift --format json

uv run fact-file-query-tool drift --file path/to/file.py
```
...
**Agent file**: [drift-detector](../../agents/drift-detector.md)
...
**Invocation**:
```bash
> Use the drift-detector agent to verify my code before committing
```
...
Task(subagent_type="drift-detector", ...)
...
- **Agent**: [drift-detector](../../agents/drift-detector.md)
