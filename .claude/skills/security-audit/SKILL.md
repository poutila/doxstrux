---
name: security-audit
description: Comprehensive security audit before committing. Use proactively to catch hardcoded secrets (API keys, passwords, tokens), dangerous calls (eval, exec), shell injection risks, SQL injection patterns, and 15+ security anti-patterns. Critical for preventing credential leaks.
allowed-tools: Bash, Read, Task
---

# Security Audit Skill

**This skill invokes the [security-audit](../../agents/security-audit.md) agent.**
...
This skill finds **15+ security anti-patterns**:
...
```bash
> Use the security-audit agent to verify my code
```

**Examples**:
```bash
> Use the security-audit agent before committing

> Have the security-audit agent check for issues

> Ask the security-audit agent to analyze the codebase
```
...
Task(
    ...
    prompt="Run security-audit analysis and report all findings with exact locations."
)
```
...
uv run fact-file-query-tool security
...
# Run validation
echo "🔍 Running Security Audit..."
uv run fact-file-query-tool security --format summary
echo "✅ Security Audit passed"

uv run fact-file-query-tool security --format json

uv run fact-file-query-tool security --file path/to/file.py
```
...
**Agent file**: [security-audit](../../agents/security-audit.md)
...
**Invocation**:
```bash
> Use the security-audit agent to verify my code before committing
```
...
- **Agent**: [security-audit](../../agents/security-audit.md)
