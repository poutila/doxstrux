...
description: Detect magic numbers during refactoring. Use during code reviews to identify unexplained numeric literals that should be named constants. Improves code readability and maintainability.
allowed-tools: Bash, Read, Task
---

# Magic Number Detector Skill

**This skill invokes the [magic-number-detector](../../agents/magic-number-detector.md) agent.**
...
Unexplained numeric literals (except 0, 1, -1, 2):
- `if age > 18:` ← What is 18?
- `timeout = 30` ← What is 30?
- `buffer_size = 4096` ← Why 4096?
...
```bash
> Use the magic-number-detector agent to verify my code
```

**Examples**:
```bash
> Use the magic-number-detector agent before committing

> Have the magic-number-detector agent check for issues

> Ask the magic-number-detector agent to analyze the codebase
```
...
```python
Task(
    subagent_type="magic-number-detector",
    description="Check code quality",
    prompt="Run magic-number-detector analysis and report all findings with exact locations."
)
```
...
uv run fact-file-query-tool magic-numbers
echo "✅ Magic Number Detector passed"
```
...
# Run validation
echo "🔍 Running Magic Number Detector..."
uv run fact-file-query-tool magic-numbers --format summary
echo "✅ Magic Number Detector passed"

uv run fact-file-query-tool magic-numbers --format json

uv run fact-file-query-tool magic-numbers --file path/to/file.py
```
...
**Agent file**: [magic-number-detector](../../agents/magic-number-detector.md)
...
**Invocation**:
```bash
> Use the magic-number-detector agent to verify my code before committing
```
...
Task(subagent_type="magic-number-detector", ...)
...
- **Agent**: [magic-number-detector](../../agents/magic-number-detector.md)
