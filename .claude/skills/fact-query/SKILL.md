---
name: fact-query
description: Query FACTS.parquet for deterministic code analysis. Use when analyzing Python code structure, complexity, security, or dependencies instead of reading files. 230 fact keys cover signatures, call graphs, errors, types, docs. Reading = guessing, facts = truth. MANDATORY - regenerate after code edits.
allowed-tools: Bash, Read, Glob, Grep, Task
---

# Fact Query Skill

**This skill invokes the [fact-query](#) agent. *(agent file not found)***
...
```bash
> Use the fact-query agent to verify my code
```

**Examples**:
```bash
> Use the fact-query agent before committing

> Have the fact-query agent check for issues

> Ask the fact-query agent to analyze the codebase
```
...
```python
Task(
    subagent_type="fact-query",
    description="Check code quality",
    prompt="Run fact-query analysis and report all findings with exact locations."
)
```
...
# Run validation
echo "🔍 Running Fact Query..."
uv run fact-file-query-tool query
echo "✅ Fact Query passed"
```
...
# Run validation
echo "🔍 Running Fact Query..."
uv run fact-file-query-tool query --format summary
echo "✅ Fact Query passed"

uv run fact-file-query-tool query --format json

uv run fact-file-query-tool query --file path/to/file.py
```
...
**Agent file**: [fact-query](#) *(agent file not found)*
...
**Invocation**:
```bash
> Use the fact-query agent to verify my code before committing
```
...
- **Agent**: [fact-query](#) *(agent file not found)*
...
**Agent Status**: ⚠️ Agent file not found
