...
description: Enforce strict type coverage requirements. Use proactively before committing to enforce type safety and catch missing type annotations. Requires 100% type coverage for public APIs - all parameters typed, all return types annotated.
allowed-tools: Bash, Read, Task
---

# Type Coverage Enforcer Skill

**This skill invokes the [type-coverage-enforcer](../../agents/type-coverage-enforcer.md) agent.**
...
### 1. Parameter Type Coverage
- All function parameters must have type annotations
- No untyped parameters in public APIs
...
```bash
> Use the type-coverage-enforcer agent to verify my code
```

**Examples**:
```bash
> Use the type-coverage-enforcer agent before committing

> Have the type-coverage-enforcer agent check for issues

> Ask the type-coverage-enforcer agent to analyze the codebase
```
...
```python
Task(
    subagent_type="type-coverage-enforcer",
    description="Check code quality",
    prompt="Run type-coverage-enforcer analysis and report all findings with exact locations."
)
```
...
# Run validation
echo "🔍 Running Type Coverage Enforcer..."
uv run fact-file-query-tool type-coverage
echo "✅ Type Coverage Enforcer passed"
```
...
# Run validation
echo "🔍 Running Type Coverage Enforcer..."
uv run fact-file-query-tool type-coverage --format summary
echo "✅ Type Coverage Enforcer passed"

uv run fact-file-query-tool type-coverage --format json

uv run fact-file-query-tool type-coverage --file path/to/file.py
```
...
**Agent file**: [type-coverage-enforcer](../../agents/type-coverage-enforcer.md)
...
**Invocation**:
```bash
> Use the type-coverage-enforcer agent to verify my code before committing
```
...
- **Agent**: [type-coverage-enforcer](../../agents/type-coverage-enforcer.md)
