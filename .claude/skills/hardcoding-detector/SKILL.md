...
description: Detect hardcoded values before committing. Use proactively to catch hardcoded paths (/var/log, C:\), URLs (https://api.prod.com), credentials (API keys, passwords), and connection strings. Prevents credential leaks and environment coupling.
allowed-tools: Bash, Read, Task
---

# Hardcoding Detector Skill

**This skill invokes the [hardcoding-detector](../../agents/hardcoding-detector.md) agent.**
...
### 1. Hardcoded File Paths (ERROR)
- Absolute paths: `/var/log/app.log`, `C:\Program Files\`
- User-specific paths: `/home/alice/data/`
...
```bash
> Use the hardcoding-detector agent to verify my code
```

**Examples**:
```bash
> Use the hardcoding-detector agent before committing

> Have the hardcoding-detector agent check for issues

> Ask the hardcoding-detector agent to analyze the codebase
```
...
Task(
    ...
    prompt="Run hardcoding-detector analysis and report all findings with exact locations."
)
```
...
# Run validation
echo "🔍 Running Hardcoding Detector..."
uv run fact-file-query-tool hardcoding
echo "✅ Hardcoding Detector passed"
```
...
# Run validation
echo "🔍 Running Hardcoding Detector..."
uv run fact-file-query-tool hardcoding --format summary
echo "✅ Hardcoding Detector passed"

uv run fact-file-query-tool hardcoding --format json

uv run fact-file-query-tool hardcoding --file path/to/file.py
```
...
**Agent file**: [hardcoding-detector](../../agents/hardcoding-detector.md)
...
**Invocation**:
```bash
> Use the hardcoding-detector agent to verify my code before committing
```
...
- **Agent**: [hardcoding-detector](../../agents/hardcoding-detector.md)
