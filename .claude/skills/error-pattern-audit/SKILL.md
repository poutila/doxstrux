...
description: Detect error handling anti-patterns before committing. Use proactively to catch bare except statements, silent exception swallowing, missing error context, and improper exception handling that makes debugging difficult.
allowed-tools: Bash, Read, Task
---

# Error Pattern Audit Skill

**This skill invokes the [error-pattern-audit](../../agents/error-pattern-audit.md) agent.**
...
1. **Bare except** - `except:` without exception type
2. **Silent swallowing** - Empty except blocks
3. **Too broad** - `except Exception:` catching everything
4. **Missing context** - Exceptions without messages
5. **Wrong exception types** - Using generic exceptions
...
```bash
> Use the error-pattern-audit agent to verify my code
```

**Examples**:
```bash
> Use the error-pattern-audit agent before committing

> Have the error-pattern-audit agent check for issues

> Ask the error-pattern-audit agent to analyze the codebase
```
...
Task(
    ...
    prompt="Run error-pattern-audit analysis and report all findings with exact locations."
)
```
...
# Run validation
echo "🔍 Running Error Pattern Audit..."
uv run fact-file-query-tool error-patterns
echo "✅ Error Pattern Audit passed"
```
...
# Run validation
echo "🔍 Running Error Pattern Audit..."
uv run fact-file-query-tool error-patterns --format summary
echo "✅ Error Pattern Audit passed"

uv run fact-file-query-tool error-patterns --format json

uv run fact-file-query-tool error-patterns --file path/to/file.py
```
...
**Agent file**: [error-pattern-audit](../../agents/error-pattern-audit.md)
...
**Invocation**:
```bash
> Use the error-pattern-audit agent to verify my code before committing
```
...
- **Agent**: [error-pattern-audit](../../agents/error-pattern-audit.md)
