...
description: Check code governance compliance before committing. Use proactively before git commits to verify compliance with complexity limits, documentation standards, error handling rules, type coverage, security patterns, and SOLID principles. Enforces all GOVERNANCE_RULES.yaml rules.
...
This skill validates compliance across **6 governance categories**:
...
2. **Documentation Standards**
   ...
   - No missing documentation for exported functions
...
5. **Security Patterns**
   ...
   - No dangerous calls (eval, exec)
   - Input validation present

6. **SOLID Principles**
   - Single Responsibility (no god classes)
   - Liskov Substitution (no NotImplementedError)
   - Reasonable coupling (fan-out ≤ 10)
...
**Examples**:
```bash
> Use the governance-check agent before committing

> Have the governance-check agent check for issues

> Ask the governance-check agent to analyze the codebase
```
...
Task(
    ...
    prompt="Run governance-check analysis and report all findings with exact locations."
)
```
