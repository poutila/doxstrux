...
3. Fail with a clear message if `rg` is missing (this is a configuration error, not a skip condition).
...
# Single override point for CI/exotic environments
...
def require_ripgrep() -> None:
    ...
    if shutil.which(RG_CMD) is None:
        raise RuntimeError(
            f"ripgrep (rg) is required for this test/tool. "
            f"Command '{RG_CMD}' not found. "
...
The helper in §3 already uses `RG_CMD` with a default of `"rg"`:

```python
RG_CMD = os.environ.get("DOXSTRUX_RG", "rg")
```

This is the single override point. All `rg` invocations flow through `run_rg()` or use `RG_CMD` directly.
