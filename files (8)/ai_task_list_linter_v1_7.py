#!/usr/bin/env python3
"""
ai_task_list_linter_v1_7.py

Deterministic linter for AI Task Lists (Spec v1.5).

Version 1.7 changes (bakes in all governance rules):
- NEW: Clean Table checklist enforcement (R-ATL-042) - 5 required prompts in STOP
- NEW: Import hygiene enforcement for Python (R-ATL-063) - rg patterns in Global Clean Table
- Spec version bump to 1.5

Version 1.6 features retained:
- R-ATL-001: schema_version == "1.5", search_tool required
- R-ATL-023: STOP labels required with real output
- R-ATL-041: No Weak Tests checklist enforcement
- R-ATL-050: Phase Unlock Artifact cat > pattern
- R-ATL-071/072: Runner/UV enforcement on $ lines
- R-ATL-075: $ prefix mandatory in key sections
...
def _parse_front_matter(lines: List[str]) -> Tuple[Dict[str, str], Optional[LintError], int]:
    ...
    # Fix B: Enforce schema_version == "1.5" exactly
    if meta["schema_version"] != "1.5":
        return meta, LintError(1, "R-ATL-001", f"schema_version must be '1.5' (found '{meta['schema_version']}')."), end_idx + 1
...
def lint(path: Path, require_captured_evidence: bool = False) -> Tuple[Dict[str, str], List[LintError]]:
    ...
    # R-ATL-050: Phase Unlock Artifact content enforcement (Gap E fix: require cat > pattern)
    ...
    if p_bounds:
        ...
        # Must have placeholder rejection scan (rg command checking for [[PH: or similar)
        has_placeholder_scan = (
            "rg" in sec_text and ("[[PH:" in sec_text or "\\[\\[PH" in sec_text)
        )
        ...
        if meta["mode"] == "template":
            ...
            artifact_pattern = re.search(r"\.phase-[N0-9]+\.complete", sec_text, re.IGNORECASE)
            ...
            cat_artifact_pattern = re.search(r"cat\s*>\s*\.phase-\d+\.complete", sec_text, re.IGNORECASE)
            if not cat_artifact_pattern:
                errors.append(LintError(content_start + 1 + p_start, "R-ATL-050",
                    "Phase Unlock Artifact must use 'cat > .phase-N.complete.json' pattern (not echo or prose mention)."))
            ...
            # Should have placeholder rejection scan
            if not has_placeholder_scan:
                errors.append(LintError(content_start + 1 + p_start, "R-ATL-050",
                    "Phase Unlock Artifact should include placeholder rejection scan (rg '\\[\\[PH:' ...)."))
    ...
    if g_bounds:
        ...
        if meta["mode"] == "template":
            ...
            runner = meta.get("runner", "")
            if runner == "uv":
                # Check for multi-dot relative import pattern
                has_multidot_check = bool(re.search(r"rg\s+['\"]from\s*\\\.\\\.?|rg\s+.*from\s*\.\.", sec_text))
                # Check for wildcard import pattern  
                has_wildcard_check = bool(re.search(r"rg\s+['\"]import\s*\\\*|rg\s+.*import\s*\*", sec_text))
                
                if not has_multidot_check:
                    errors.append(LintError(content_start + 1 + g_start, "R-ATL-063",
                        "Python project (runner=uv) missing import hygiene check: rg 'from \\.\\.' pattern for multi-dot relative imports."))
                if not has_wildcard_check:
                    errors.append(LintError(content_start + 1 + g_start, "R-ATL-063",
                        "Python project (runner=uv) missing import hygiene check: rg 'import \\*' pattern for wildcard imports."))
...
def main(argv: List[str]) -> int:
    parser = argparse.ArgumentParser(prog="ai_task_list_linter_v1_7.py")
