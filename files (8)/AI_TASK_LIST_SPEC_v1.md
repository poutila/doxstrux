# AI Task List Spec v1.5

**Spec ID**: `AI_TASK_LIST_SPEC_V1_5`  
**Schema version**: `1.5`  
...
8. **NEW v1.5**: All governance rules baked in (import hygiene, Clean Table checklist)
...
### R-ATL-001: Front matter required — UPDATED in v1.5
...
ai_task_list:
  ...
  search_tool: "rg" | "grep"  # Required in v1.4+ (was optional)
```

**v1.5 changes**:
- `schema_version` must be exactly `"1.5"` (linter enforces this)
- All governance rules now baked in
...
- Instantiated mode: 
  - Section MUST use `cat > .phase-N.complete.json` pattern (not echo or prose mention)
  - Section MUST include a fenced code block with artifact generation commands
  - Section MUST include placeholder rejection scan (`rg '\[\[PH:' ...`)
  - Trivial commands like `echo .phase-0.complete.json` do NOT satisfy this requirement

**Rationale**: The Phase Unlock Artifact is the gate between phases. Without explicit `cat >` artifact creation and placeholder verification, the phase gating contract becomes ceremonial and unenforceable.
...
### R-ATL-063: Import hygiene enforcement (Python projects) — NEW in v1.5

When `runner: "uv"` (Python project) and `mode: instantiated`, the linter MUST verify that the Global Clean Table Scan section contains:

1. Multi-dot relative import check: `rg 'from \.\.'` pattern
2. Wildcard import check: `rg 'import \*'` pattern
...
**Rationale**: Import hygiene rules (absolute imports preferred, no multi-dot relative, no wildcards) are critical for Python projects. When `runner=uv` indicates a Python project, these checks become mandatory to prevent coupling and improve refactorability.
...
A document is **Spec v1.5 compliant** if:

1. ✅ schema_version must be exactly "1.5" (R-ATL-001)
2. ✅ search_tool is required (not optional) (R-ATL-001)
...
9. ✅ STOP evidence has BOTH labels with real output (R-ATL-023)
10. ✅ No Weak Tests checklist: 4 required prompts (R-ATL-041)
11. ✅ **NEW**: Clean Table checklist: 5 required prompts (R-ATL-042)
12. ✅ Phase Unlock Artifact uses `cat > .phase-N.complete` pattern (R-ATL-050)
13. ✅ $ prefix mandatory for command lines in key sections (R-ATL-075)
14. ✅ Drift Ledger Evidence contains path:line witness in instantiated mode (R-ATL-D3)
15. ✅ search_tool=rg forbids grep in code blocks only (R-ATL-D4)
16. ✅ Runner prefix enforced on $ command lines (R-ATL-071)
17. ✅ UV forbidden patterns only on $ command lines (R-ATL-072)

**Optional (with --require-captured-evidence)**:

18. ⚡ Captured evidence headers (`# cmd:`, `# exit:`) in all evidence blocks (R-ATL-024)
19. ⚡ Exit code 0 required for STOP and Global Clean Table evidence

**Recommended (project-specific Clean Table patterns)**:

20. 📋 No unfinished markers: `rg 'TODO|FIXME|XXX' src/` (R-ATL-062)

**Python-specific (runner=uv) — LINTER ENFORCED**:

21. ✅ **NEW**: Import hygiene: `rg 'from \.\.' src/` required in Global Clean Table (R-ATL-063)
22. ✅ **NEW**: Import hygiene: `rg 'import \*' src/` required in Global Clean Table (R-ATL-063)
