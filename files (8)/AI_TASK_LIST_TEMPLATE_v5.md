...
**Version**: 5.0 (v1.5 spec — all governance baked in)  
...
```bash
$ rg '\[\[PH:[A-Z0-9_]+\]\]' PROJECT_TASKS.md && { echo "ERROR: Placeholders found"; exit 1; } || exit 0
```
...
**Verification**:
```bash
rg '\[\[PH:[A-Z0-9_]+\]\]' PROJECT_TASKS.md && exit 1 || true
...
rg '\[\[PH:|YYYY-MM-DD|TBD' .phase-0.complete.json && exit 1 || true
```
...
$ rg '\[\[PH:|YYYY-MM-DD|TBD' .phase-N.complete.json && exit 1 || true
```
...
# $ rg 'TODO|FIXME|XXX' src/ && exit 1 || true           # No unfinished markers
# $ rg '\[\[PH:' . && exit 1 || true                     # No placeholders
#
...
$ rg 'from \.\.' [[PH:SOURCE_DIR]] && exit 1 || true
$ rg 'import \*' [[PH:SOURCE_DIR]] && exit 1 || true
