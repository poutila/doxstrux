...
Small deterministic linter for AI Task Lists (Spec v1) with **minimal updates**:
- D2: enforce Preconditions symbol-check command presence for non-Phase-0 tasks
- D3: enforce Drift Ledger Evidence witness format (path:line) for non-empty rows

Everything else remains as in v1:
- stdlib only
            ...
            if pre_i is None:
                ...
                if fenced is None:
                    ...
                    if meta["mode"] == "template":
                        ...
                        # instantiated: require grep/rg pattern in Preconditions commands
                        if not any(RE_SYMBOL_CMD.search(ln) for ln in code_lines):
